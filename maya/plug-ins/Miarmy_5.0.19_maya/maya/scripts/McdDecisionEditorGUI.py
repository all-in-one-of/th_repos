...
def McdDecisionEditorGUI():
    ...
    cmds.menuItem( label='Switch Smooth Filter', c = "switchOldMode()" )
    cmds.menuItem( label='Switch On / Off', c = "switchDecisionOnOff()" )
    cmds.menuItem( label='Switch Abs Mode', c = "switchDecisionMode()" )
    cmds.menuItem( label='Switch Expression Mode', c = "switchExpressionMode()")
    cmds.menuItem( label='Switch Full Fuzzy Pass Down Mode', c = "switchFullFuzzyMode()")
    cmds.menuItem(divider = True)
    cmds.menuItem( label='Language Mode', c = "switchLggMode()")
    cmds.menuItem( label='Channel Mode', c = "switchChMode()")
    ...
    cmds.tabLayout( tabs, edit=True,tabLabel=((child0, "Logic Table"),(child1, "Logic Tips")))
...
def buildDecisionGUIContents():
    ...
    if selObj != [] and selObj != None:
        ...
        if cmds.nodeType(dcNode) == "McdDecision" :
            ...
            isProxied = checkDecisionIsProxied(dcNode)
            if isAct == 1:
    ...
    # melCmd = "McdSimpleCommand -exe 16;"
    # mel.eval(melCmd)
    
    
    isEnableSmooth = False
    try:
        if cmds.getAttr(dcNode + ".lggmode") == True:
            isEnableSmooth = True
    except:
        pass
    ...
    isLggMode = True
    if cmds.getAttr(globalNode + ".boolMaster[16]") == True:
        isLggMode = False
    ...
    cmds.rowColumnLayout("gui_dc_1", nc = 12, cw = [(1,100),(2,100),(3,20),(4,80),(5,120),(6,20),(7,150), (8, 20), (9, 90), (10, 90), (11, 90), (12, 90)])
    ...
    try:
        if cmds.getAttr(dcNode + ".enable") != 1:
            cmds.text(l = "Turned Off", bgc = [1, .4, .4])
    except:
        pass
    skipFuzzy = False
    try:
        skipFuzzy = cmds.getAttr(dcNode + ".skipFuzzy")
        if skipFuzzy != 0:
            cmds.text(l = "Expression", bgc = [1, 1, .2])
    except:
        pass
    fullFuzzy = False
    try:
        fullFuzzy = cmds.getAttr(dcNode + ".fullFuzzy")
        if fullFuzzy != 0:
            cmds.text(l = "Full Fuzzy", bgc = [.65, .4, 1])
    except:
        pass
    
    enableWhenNotSkipFuzzy = True
    if skipFuzzy:
        enableWhenNotSkipFuzzy = False
    ...
    if isLggMode:
        ...
        if decisionCondition != 2:
            cmds.rowColumnLayout("gui_dc_2", nc = 11, cw = [(1,20),(2,45),(3,40),(4,25),(5,30),(6,400),\
                                                            (7,50),(8,50),(9, 188),(10,30),(11,30)])
            cmds.text(l = "", fn = "smallBoldLabelFont", align = "left")
            ...
            cmds.text(l = "Input Condition (Fuzzy):", fn = "smallBoldLabelFont", align = "left")
                    ...
                    if isAct == 1:
                        ...
                        if isDefault < 1:
                            ...
                            for i in range(20):
                                ...
                                dId = getIdString(i)
                                dInputLgg = cmds.getAttr(dcNode + ".inputLanguage[" +stri+ "]")
                                ...
                                if dActive == 0:
                                    ...
                                    cmds.textField("if_dn_inputLgg" + stri, en = False) # input
                                    ...
                                    cmds.textField("if_dn_inputLgg" + stri, tx = dInputLgg, cc = "tf_dns_inputCCLgg(" + stri + ")") # input
                                    
                                    McdLanguageMenu(i, dcNode, "if_dn_inputLgg")
                                    
                                    if dInfMin == True:
                                        if dAutoFillFzOut == 0:
                                            cmds.floatField("if_dn_fzmin" + stri,v = dFuzzyIn, pre = 2, cc = "ff_inputFVMinCC(" + stri + ")", en = enableWhenNotSkipFuzzy, vis = False)#fuzzy min
                                        else:
                                            cmds.floatField("if_dn_fzmin" + stri,v = dFuzzyIn, pre = 2, cc = "ff_inputFVMinCC2(" + stri + ")", en = enableWhenNotSkipFuzzy, vis = False)#fuzzy min
                                    else:
                                        if dAutoFillFzOut == 0:
                                            cmds.floatField("if_dn_fzmin" + stri,v = dFuzzyIn, pre = 2, cc = "ff_inputFVMinCC(" + stri + ")", en = enableWhenNotSkipFuzzy)#fuzzy min
                                        else:
                                            cmds.floatField("if_dn_fzmin" + stri,v = dFuzzyIn, pre = 2, cc = "ff_inputFVMinCC2(" + stri + ")", en = enableWhenNotSkipFuzzy)#fuzzy min
                                    if dInfMax == True:
                                        cmds.floatField("if_dn_fzmax" + stri,v = dFuzzyOut, pre = 2, cc= "ff_inputFVMaxCC(" + stri + ")", en = enableWhenNotSkipFuzzy, vis = False)#fuzzy min
                                    else:
                                        cmds.floatField("if_dn_fzmax" + stri,v = dFuzzyOut, pre = 2, cc= "ff_inputFVMaxCC(" + stri + ")", en = enableWhenNotSkipFuzzy)#fuzzy min
                                    ...
                                    if i == 0:
                                        ...
                                        cmds.button(l = "Up", c = "moveUpSentence(" + stri + ", \"" + dcNode + "\", 1)", en = enableWhenNotSkipFuzzy)
                                    cmds.button(l = "Del", c = "deleteSentence(" + stri + ", \"" + dcNode + "\")", en = enableWhenNotSkipFuzzy)
                                    ...
                                    if not dInfMin:
                                        ...
                                        cmds.floatField("gfpos1" + stri, v = gfvalues[0], pre = 2, en = False, vis = False) # graphic
                                        cmds.floatField("gfpos2" + stri, v = gfvalues[1], pre = 2, en = False, vis = False) # graphic
                                    if not dInfMax:
                                        ...
                                        cmds.floatField("gfpos3" + stri, v = gfvalues[2], pre = 2, en = False, vis = False) # graphic
                                        cmds.floatField("gfpos4" + stri, v = gfvalues[3], pre = 2, en = False, vis = False) # graphic
                                    cmds.setParent("..")
                                    cmds.text(l = "")
                                    cmds.text(l = "")
                    else:
                            ...
                            for i in range(20):
                                ...
                                if dActive == 0:
                                    ...
                                    cmds.checkBox("cd_dn_infMin" + stri, l = "", v = dInfMin, onc = "cb_dns_infMin_on(" + stri + ")",\
                                                                                            ...
                                                                                            en = enableWhenNotSkipFuzzy )
                                    if dInfMin == True:
                                        cmds.text(l = "- infinity", en = enableWhenNotSkipFuzzy)
                                    else:
                                        cmds.floatField("if_dn_min" + stri, v = dMin, pre = 2, cc = "ff_inputVMinCC(" + stri + ")",\
                                                        en = enableWhenNotSkipFuzzy) # min
                                    ...
                                    if dInfMax == True:
                                        ...
                                        cmds.floatField("if_dn_max" + stri, v = dMax, pre = 2, cc = "ff_inputVMaxCC(" + stri + ")", \
                                                        en = enableWhenNotSkipFuzzy) # max
                                    if dInfMin == True:
                                        cmds.text(l = "")
                                    else:
                                        if dAutoFillFzOut == 0:
                                    ...
                                    cmds.optionMenu("if_dn_inTyp" + stri, cc = "om_dn_inTyp(" + stri + ")", en = enableWhenNotSkipFuzzy) #input type
                                    setupOmDecisionInTyp(dInputType, stri)
                                    cmds.button(l = "Auto Fill", c = "autoFillSentence(" + stri + ", \"" + dcNode + "\")", en = enableWhenNotSkipFuzzy)
                                    if i == 0:
                                        cmds.text(l = "")
                                    else:
                                        cmds.button(l = "Up", c = "moveUpSentence(" + stri + ", \"" + dcNode + "\", 1)", en = enableWhenNotSkipFuzzy)
    ...
    if isLggMode:
        ...
        # for output decision:
        if isDefault == 0:
            cmds.rowColumnLayout("gui_dc_4", nc = 4, cw = [(1,20), (2,400), (3,60), (4,60)])
            cmds.text(l = "", fn = "smallBoldLabelFont", align = "left")
            cmds.text(l = "Output Decision", fn = "smallBoldLabelFont", align = "left")
            cmds.text(l = "Smooth", fn = "smallBoldLabelFont", align = "left")
            cmds.text(l = "")
        else:
            # default:
            cmds.rowColumnLayout("gui_dc_4", nc = 2, cw = [(1,20),(2,400)])
            cmds.text(l = "On", fn = "smallBoldLabelFont", align = "left")
            cmds.text(l = "Default Decision", fn = "smallBoldLabelFont", align = "left")
        ...
        if selObj != [] and selObj != None:
            ...
            if cmds.nodeType(dcNode) == "McdDecision" and nonActive == False:
                ...
                for i in range(20):
                    ...
                    dOutLgg = cmds.getAttr(dcNode + ".outLgg[" +stri+ "]")
                    ...
                    if dOutputActive == 0:
                        ...
                        cmds.textField("tf_dn_outLgg" + stri, en = False) # output decison:
                        ...
                        cmds.textField("tf_dn_outLgg" + stri, tx = dOutLgg, cc = "tf_dn_outLggCC(" + stri + ")") # output decision
                        
                        McdOutLanguageMenu(i, dcNode, "if_dn_outputLgg")
                        if isDefault != 1:
                            if enableWhenNotSkipFuzzy:
                                if isEnableSmooth:
                                    cmds.intField("ff_dn_frt" + stri, v = dOutputFilter, cc = "om_dn_outFilter(" + stri + ")", min = 0, max = 99, en = enableWhenNotSkipFuzzy) # output value
                                    cmds.text(l = "% Percent", en = enableWhenNotSkipFuzzy)
                                else:
                                    cmds.intField("ff_dn_frt" + stri, v = dOutputFilter, cc = "om_dn_outFilter(" + stri + ")", min = 0, max = 99, en = False) # output value
                                    cmds.text(l = "% Percent", en = False)
                            else:
                                cmds.intField("ff_dn_frt" + stri, v = dOutputFilter, cc = "om_dn_outFilter(" + stri + ")", min = 0, max = 99, en = enableWhenNotSkipFuzzy) # output value
                                cmds.text(l = "% Percent", en = enableWhenNotSkipFuzzy)
        ...
        if isDefault == 0:
            ...
            cmds.text(l = "", fn = "smallBoldLabelFont", align = "left")
            cmds.text(l = "THEN make decision", fn = "smallBoldLabelFont", align = "left")
            ...
            cmds.rowColumnLayout("gui_dc_4", nc = 5, cw = [(1,20),(2,200),(3,80),(4,100),(5, 50)])
            cmds.text(l = "On", fn = "smallBoldLabelFont", align = "left")
            cmds.text(l = "Default Decision", fn = "smallBoldLabelFont", align = "left")
            cmds.text(l = "Default Value", fn = "smallBoldLabelFont", align = "left")
            ...
            if cmds.nodeType(dcNode) == "McdDecision" and nonActive == False:
                ...
                for i in range(20):
                    ...
                    if dOutputActive == 0:
                        ...
                        cmds.floatField("ff_dn_ov" + stri, v = dOutputValue, pre = 2, cc = "ff_dn_ovCC(" + stri + ")", en = enableWhenNotSkipFuzzy) # output value
                        if isDefault == 1:
                        ...
                        cmds.button(l = "Auto Fill", c = "autoFillDecision(" + stri + ", \"" + dcNode + "\")", en = enableWhenNotSkipFuzzy)
                        if isDefault != 1:
                            cmds.intField("ff_dn_frt" + stri, v = dOutputFilter, cc = "om_dn_outFilter(" + stri + ")", min = 0, max = 99, en = enableWhenNotSkipFuzzy) # output value
                            cmds.text(l = "% Percent", en = enableWhenNotSkipFuzzy)
    ...
    if isProxied: 
        cmds.text("gui_dc_na", l = "\nProxied decision node selected, please select the source decision node of this node.\n ", bgc = [.3, .6, 1.0])
        cmds.text("gui_dc_na2", l = "")
        cmds.rowColumnLayout("gui_dc_6", nc = 3, cw = [(1,400 ),(2,200),(3,200)])
        cmds.text(l = "")
        cmds.button(l = "Select Incoming Source Decision", h = 50, bgc = [0.15, 0.7, 0.3], c = "McdSelectSourceDcNode('" + selDcNode + "')")
        cmds.text(l = "")
        cmds.setParent("..")
    elif nonActive:
        ...
        cmds.rowColumnLayout("gui_dc_6", nc = 3, cw = [(1,400 ),(2,200),(3,200)])
        cmds.text(l = "")
        cmds.button(l = "Click and Active Now", h = 50, bgc = [0.15, 0.7, 0.3], c = "McdActiveNonactiveNode('" + selDcNode + "')")
        cmds.text(l = "")
        cmds.setParent("..")
...
def McdBuildBrainShadow():
    # must select all decision from others agent type:
    isRef = False
    try:
        agentGrp = cmds.ls(type = "McdAgentGroup")[0]
        if agentGrp.find(":") > 0:
            isRef = True 
    except:
        pass
    ...
    if MIsBlank(selObjs):
        cmds.confirmDialog(t = "Error", m = "Please select at least one decision node from other type of agent")
        return
    
    for i in range(len(selObjs)):
        if cmds.nodeType(selObjs[i]) != "McdDecision":
            cmds.confirmDialog(t = "Error", m = "One decision node you selected is not a decision node")
            return
        
    isAct = mel.eval("McdCheckNodeActiveCmd;")
    
    if isAct:
        cmds.confirmDialog(t = "Error", m = "Please select the node belongs to non active agent types")
        return
    
    activeAgentName = McdGetActiveAgentName()
    if isRef:
        pNodeTemp = activeAgentName + ":Decision_*"
        pNodeRaw = cmds.ls(pNodeTemp)
        if not MIsBlank(pNodeRaw):
            pNode = pNodeRaw[0]
    else:    
        pNode = "Decision_" + activeAgentName
        
        
    pNodeList = cmds.ls(pNode)
    if len(pNodeList) != 1:
        cmds.confirmDialog(t = "Error", m = "Cannot find correct \"" + pNode + "\"")
        return
    
    dupObjs = cmds.duplicate(rr = True)
    cmds.parent(dupObjs, pNode)
    
    # link:
    for i in range(len(dupObjs)):
        allChild1 = cmds.listRelatives(selObjs[i], c = True, p = False, ad = True, f = True)
        allChild2 = cmds.listRelatives(dupObjs[i], c = True, p = False, ad = True, f = True)
        if not MIsBlank(allChild1) and not MIsBlank(allChild2):
            for j in range(len(allChild1)):
                cmds.connectAttr(allChild1[j] + ".enable", allChild2[j] + ".enable")
        cmds.connectAttr(selObjs[i] + ".enable", dupObjs[i] + ".proxy")
        
    
    # rename and make neat!
    for i in range(len(dupObjs)):
        allChild = cmds.listRelatives(dupObjs[i], c = True, p = False, ad = True, f = True)
        if not MIsBlank(allChild):
            for j in range(len(allChild)):
                childNameReal = allChild[j].split("|")[-1]
                newNamePre = childNameReal.split("_decision_")[0]
                newName = newNamePre + "_decision_" + activeAgentName
                cmds.rename(allChild[j], newName)
            
        newNamePre = dupObjs[i].split("_decision_")[0]
        newName = newNamePre + "_decision_" + activeAgentName
        cmds.rename(dupObjs[i], newName)
        
    
def checkDecisionIsProxied(dcNode):
    
    allConns = cmds.listConnections(dcNode, s = True, d = False, p = True)
    if not MIsBlank(allConns):
        for i in range(len(allConns)):
            if allConns[i].find(".enable") > 0:
                return True
