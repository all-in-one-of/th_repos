...
def McdRearrangeDuplicatedMeshes():
    ...
    dupRoot = cmds.ls("McdRoot_*")
    if not MIsBlank(dupRoot):
        cmds.confirmDialog(t = "Error", m = "Already re-arranged. Please clean and duplicate mesh, and then re-arrange again.")
        return
            ...
            for j in range(len(allChild)):
                ...
                for k in range(len(geoNameList)):
                    if shapeName.find(geoNameList[k]) >= 0:
                        ...
                        if (actionNameList[k] == action) and (frame >= startFrameList[k]) and (frame <= endFrameList[k]):
                            foundThisNode = True
                            break
                if foundThisNode:
