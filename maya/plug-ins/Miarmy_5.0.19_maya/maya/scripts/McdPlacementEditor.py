...
def ReadProportion(allAgents):
    ...
    for i in range(len(allAgtGrp)):
        stri = str(i)
        pWeight = cmds.getAttr(placeNode + ".pWeight[" + stri + "]")
        proportionWList.append(pWeight)
            ...
            try:
                ...
                for j in range(len(proportionList)/2):
                    if agentName == proportionList[j*2]:
                        ...
                        cmds.floatField("weightIF" + stri, e = True, v = proportionWList[i])
...
def changePPWeight(index):
    stri = str(index)
    weightVal = cmds.floatField("weightIF" + stri, q = True, v = True)
    ...
    #print placeNode
    #print placeNode + ".pWeight[" + stri + "]"
    cmds.setAttr(placeNode + ".pWeight[" + stri + "]", weightVal)
    
    
def updatePPFromWeight():
    ...
    for i in range(len(allAgtGrp)):
        ...
        if pWeight > 0.0001:
            allIDList.append(i)
            allWeights.append(pWeight)
            sumWeight += pWeight
        
    if sumWeight == 0:
        return
    
    for i in range(len(allIDList) + 1):
        stri  = str(i * 3)
        stri1 = str(i * 3 + 1)
        stri2 = str(i * 3 + 2)
        ...
        if i == len(allIDList):
            cmds.setAttr(placeNode + ".proportion[" + stri + "]", 0 )
            cmds.setAttr(placeNode + ".proportion[" + stri1 + "]", 0 )
            cmds.setAttr(placeNode + ".proportion[" + stri2 + "]", 0 )
        else:
            cmds.setAttr(placeNode + ".proportion[" + stri + "]", allIDList[i] )
            cmds.setAttr(placeNode + ".proportion[" + stri1 + "]", allIDList[i] )
            cmds.setAttr(placeNode + ".proportion[" + stri2 + "]", float(allWeights[i]) / float(sumWeight) )


    # updating the slider and proportion val:
    UpdateProportion()
...
def clearPPWeightInfo():
    ...
    for i in range(100):
        stri = str(i)
        cmds.setAttr(placeNode + ".pWeight[" + stri + "]", 0)
...
def addPlaceOffsetRecord(chID, opt):
    ...
    if optIndex == -1:
        ...
        cmds.setAttr(placeNode + ".userOffset[" + str(optIndex * 8 + 7) + "]", -1)
...
def setIndiAgentType():
    ...
    stat = cmds.promptDialog(t = "Agent Type ID", m = "Please Specify the Agent Type ID", \
                    button = ["OK", "Cancel"], \
                    defaultButton = "OK", cancelButton = "Cancel", dismissString = "Cancel")
    if stat == "OK":
        try:
            tid = float(cmds.promptDialog(query = True, text = True))
        except:
            return
    if stat == "Cancel":
        return;
    ...
    # operate on optIndex
    cmds.setAttr(placeNode + ".userOffset[" + str(optIndex * 8 + 7) + "]", tid)
...
def clearCurrentRecord():
    ...
    # operate on optIndex
    for i in range(6):
        cmds.setAttr(placeNode + ".userOffset[" + str(optIndex * 8 + i + 1) + "]", 0)     # 1-6
    cmds.setAttr(placeNode + ".userOffset[" + str(optIndex * 8 + 6 + 1) + "]", -1)        # 7
...
def clearAllRecord():
    ...
    for k in range(len(collectIds)):
        for i in range(6):
            cmds.setAttr(placeNode + ".userOffset[" + str(k * 8 + i + 1) + "]", 0)     # 1-6
        cmds.setAttr(placeNode + ".userOffset[" + str(k * 8 + 6 + 1) + "]", -1)        # 7
