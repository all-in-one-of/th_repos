...
def discounnectNReconnectMDTime():
    
    mdNode = cmds.ls(type = "McdMeshDriveIM")[0]
    allConns = cmds.listConnections(mdNode, s = True, d = False)
    if MIsBlank(allConns):
        return

    try:
        cmds.disconnectAttr(allConns[0] + ".output", mdNode + ".timeValue")
        cmds.connectAttr(allConns[0] + ".output", mdNode + ".timeValue")
    except:
        pass
...
def addAuxiliaryGroup(agentName, parentName):
    ...
    cmds.group(n = "LODFar_" + agentName, em = True)
    cmds.parent("LODFar_" + agentName, parentName)
    cmds.addAttr("LODFar_" + agentName, ln = "moreThanDistance", at = "long", k = True)
    ...
    cmds.group(n = "Misc_" + agentName, em = True)
    cmds.parent("Misc_" + agentName, parentName)
...
def getSubActiveAgentName(activeAgentName):
    subActiveAgentName = ""
    agtGrpPre = activeAgentName + ":Agent_*"
    agtGrpList = cmds.ls(agtGrpPre)
    if MIsBlank(agtGrpList):
        cmds.confirmDialog(t = "Note", m = "Cannot find agent group node.")
        raise
    
    agtGrp = agtGrpList[0]
    segs = agtGrp.split("Agent_")
    if len(segs) > 1:
        subActiveAgentName = segs[1]
        
    if subActiveAgentName == "":
        cmds.confirmDialog(t = "Note", m = "Cannot find agent group node sub active name")
        raise
    
    return subActiveAgentName
    

def McdCreateStateReference(stateName, activeAgentName):
    subActiveAgentName = getSubActiveAgentName(activeAgentName)
    
    stateNameLong = activeAgentName + ":" + stateName + "_state_" + subActiveAgentName
    temp = cmds.ls(stateNameLong)
    if temp == None or temp == []:
        cmds.createNode("McdState", n = stateNameLong)
    else:
        cmds.confirmDialog(t = "Note", m = "State exist.")
    try:
        cmds.parent(stateNameLong, activeAgentName + ":State_" + subActiveAgentName)
    except:
        pass
    
def McdCreateAction(activeAgentName):
    
    count = 0
    newName = ""
    while(1):
        strc = str(count)
        newName = "ActionShell_Dummy" + strc
        result = cmds.ls(newName)
        if MIsBlank(result):
            break
        count += 1
    
    actionNameLong = newName
    cmds.createNode("McdActionShell", n = actionNameLong, ss = True)
    cmds.setAttr(actionNameLong + ".active", 1)
...
def McdCreateActionReference(activeAgentName):
    subActiveAgentName = getSubActiveAgentName(activeAgentName)
    ...
    while(1):
        strc = str(count)
        newName = activeAgentName + ":ActionShell_Dummy" + strc
        result = cmds.ls(newName)
        if MIsBlank(result):
            break
        count += 1
...
def McdCreateDecision(decisionName, activeAgentName, absMode = False):
    ...
    if isReferenceScene():
        newEngine = cmds.optionVar( q='MiarmyDnRefFlag' )
        if newEngine < 35:
            # prompt:
            stat = cmds.confirmDialog(t = "Warning", m = "You are in referenced scene, creating assets may cause pipeline confusion, continue?", \
                                                b = ["Continue", "No", "Continue, don not mention again"])
            
            # store new flag
            if stat == "Continue, don not mention again":
                cmds.optionVar( iv=('MiarmyDnRefFlag', 36) )
            elif stat == "No":
                return
...
def McdAgentContainerBuildCheckChild(agentName, subGroup):
    ...
    if subGrp != [] and subGrp != None:
        ...
        if subGroup == "State" or subGroup == "ActionShell":
            ...
            print subGroup
            if subGroup == "LODFar":
                cmds.addAttr(groupName, ln = "moreThanDistance", at = "long", k = True)
...
def getRenameHiCmdStr(prefix):
    ...
    # ~/Autodesk/Maya2012/scripts/others/prefixHierarchy.mel
    renameCommand = '\
...
def getSelection2(type = ""):
    if type == "":
        selObj = cmds.ls(sl = True)
        if MIsBlank(selObj):
            cmds.confirmDialog(t = "Error", m = "Please select the 2nd object")
            raise Exception("Please select the second object")
    ...
    selObj = cmds.ls(sl = True)
    if MIsBlank(selObj):
        cmds.confirmDialog(t = "Error", m = "Please select the 2nd " + type + " node")
        raise Exception("Please select the 2nd " + type + " node")
    
    if len(selObj) < 2:
        cmds.confirmDialog(t = "Error", m = "Please select the 2nd " + type + " node")
        raise Exception("Please select the 2nd " + type + " node")
    
    if cmds.nodeType(selObj[1]) == type:
        targetNode = selObj[1]
        return targetNode
    else:
        allChild = getAllChildren(selObj[1])
        if MIsBlank(allChild):
            cmds.confirmDialog(t = "Error", m = "Please select the 2nd " + type + " node")
            raise Exception("Please select the 2nd " + type + " node")
        ...
        try:
            if rules[i].find("include") == 0:
                ...
                for j in range(len(allObjs)):
                    ...
                    if re.match("^\d+?$", valueStr):
                        cmds.setAttr(allObjs[j] + "." + info[1], int(info[2]))
                        configInfo += allObjs[j]
                        configInfo += " " + info[1]
                        configInfo += " " + info[2]
                        configInfo += "\r\n"
                        
                    elif re.match("-|^\d+?\.\d+?$", valueStr):
...
def McdSaveLoadEnvVar():
    cmds.confirmDialog( t = "Export Initial Paths", m = "Logic: MCD_LOGIC_PATH\n" + \
                                                        "Actions: MCD_ACTION_PATH\n" + \
                                                        "Story/State Machine: MCD_STORY_PATH\n" + \
                                                        "Original Agent: MCD_OA_PATH")
...
def checkSkinForAll():
    ...
    if McdIsBlank(allGeoGrp):
        raise Exception("No object need to export")
    allGeoGrpList = []
    ...
    if allGeos == []:
        cmds.confirmDialog(t = "Error", m = "No Geometry found in Geometry_XXX node.")
        raise Exception("No Geometry found in Geometry_XXX node.")
    
    noSkinObj = False
    noSkinObjName = ""
    for i in range(len(allGeos)):
        allHis = cmds.listHistory(allGeos[i])
        if MIsBlank(allHis):
            noSkinObj = True
            break
        foundSkin = False
        for j in range(len(allHis)):
            if cmds.nodeType(allHis[j]) == "skinCluster":
                ...
                break
        if not foundSkin:
            noSkinObjName = allGeos[i]
            noSkinObj = True
            break
    
    if noSkinObj:
        cmds.confirmDialog(t = "Error", m = "Below Object isn't deformed by Skin Cluster method:\n" + noSkinObjName )
        raise Exception("Found object isn't deformed by Skin Cluster method")
...
def checkUVForAll():
    ...
    noUVGeo = []
    allGeoTransform = []
    if allGeos != [] and allGeos != None:
        for i in range(len(allGeos)):
            if cmds.getAttr(allGeos[i] + ".intermediateObject") == 1:
                continue;
            currentParents = cmds.listRelatives(allGeos[i], p = True, path = True)
            if currentParents != None and currentParents != []:
                if currentParents[0] not in allGeoTransform:
                    if currentParents[0].find("_dummyShape_") < 0:
    ...
    if allGeoTransform != []:
        for i in range(len(allGeoTransform)):
            try:
                ...
                option = cmds.confirmDialog(t = "Fatal Error", m = "Some geometries broken, system select them for you.", b = ["Ignore", "Canncel"])
                cmds.select(allGeoTransform[i])
                if option == "Canncel":
                    raise
    ...
    if noUVGeo != []:
        option = cmds.confirmDialog(t = "Fatal Error", m = "Some geometries dont have UVs, system select them for you.\n" + \
                                                "Just click CreateUVs > Planar Mapping can solve this.", b = ["Ignore", "Canncel"])
        cmds.select(noUVGeo)
        if option == "Canncel":
            raise
    
    
def McdAddLinkButton(labelStr, linkStr, hVal = 0):
    if hVal == 0:
        cmds.button(l = labelStr, c = 'cmds.launch(web="' + linkStr + '")')
    else:
        cmds.button(l = labelStr, c = 'cmds.launch(web="' + linkStr + '")', h = hVal)


def McdCheckDir(folderName, subFolder = "-------"):
    
    if subFolder == "-------":
        
        if not os.access(folderName, os.W_OK):
            
            stat = cmds.confirmDialog(t = "Question", m = "Do you want to create export folder at: \n" + \
                                        folderName,
                                        b = ["OK", "Cancel"])
            if stat != "OK":
                raise
            ...
            try:
                ...
                cmds.confirmDialog(t = "Error", m = "Cannot create folder at: \n" + folderName )
                raise
            
    else:
        
        if not os.access(folderName + "/" + subFolder, os.W_OK):
            ...
            try:
                os.makedirs(folderName + "/" + subFolder)
            except:
                cmds.confirmDialog(t = "Error", m = "Cannot create folder at: \n" + folderName + "/" + subFolder )
                raise
        
def McdInserString(pos, insertStr, inStr):
    nbChar = len(inStr)
    if pos >= nbChar:
        return None
...
def McdSetAllOldMode():
    globalNode = McdGetMcdGlobalNode()
    cmds.setAttr(globalNode + ".boolMaster[16]", True)

    

def McdSetAllLggMode():
    globalNode = McdGetMcdGlobalNode()
    cmds.setAttr(globalNode + ".boolMaster[16]", False)
        
        
def McdGetAllAgentTypeNames():
    cmds.ls(type = "McdAgentGroup")
    

def McdActiveNonactiveNode(selNode):
    currentAgentGrp = ""
    allAgtGrp = cmds.ls(type = "McdAgentGroup")
    if not MIsBlank(allAgtGrp):
        for i in range(len(allAgtGrp)):
            allChildren = cmds.listRelatives(allAgtGrp[i], ad = True, path = True)
            if selNode in allChildren:
                currentAgentGrp = allAgtGrp[i]
                break
    
    if currentAgentGrp != "":
        typeName = ""
        
        if currentAgentGrp.find(":") > 0:
            typeName = currentAgentGrp.split(":")[0]
        else:
            typeName = currentAgentGrp.split("Agent_")[-1]
        
        if typeName != "":
            globalNode = McdGetMcdGlobalNode()
            cmds.setAttr(globalNode + ".activeAgentName", typeName, type = "string")
            cmds.select(selNode)
        
        
def McdGetAllUplinks(inObj):
    allConns = cmds.listConnections(inObj, s = True, sh = True, d = False)
    return allConns
    

def McdGetAllDwLinks(inObj):
    allConns = cmds.listConnections(inObj, s = False, sh = True, d = True)
    return allConns


def McdGetAllRenderGeos():
...
def McdSetupExportFlagForSpeicalMeshes(on_off = False):
    allGeos = McdGetAllRenderGeos()
    
    for i in range(len(allGeos)):
        allConns = cmds.listConnections(allGeos[i], s = True, d = False, sh = True)
        if not MIsBlank(allConns):
            for j in range(len(allConns)):
                if cmds.nodeType(allConns[j]) == "nCloth":
                    try:
                        cmds.setAttr(allGeos[i] + ".mcdExport", on_off)
                    except:
                        print "add attr mcdExport to " + allGeos[i]
                        try:
                            cmds.addAttr(allGeos[i], at = "bool", ln = "mcdExport")
                            cmds.setAttr(allGeos[i] + ".mcdExport", cb = True)
                            cmds.setAttr(allGeos[i] + ".mcdExport", on_off)
                        except:
                            print "cannot add attr."
                    
        
        
def McdIsLoadMiarmyExpress():
    allPlugins = cmds.pluginInfo( query=True, listPlugins=True )
    for i in range(len(allPlugins)):
        if allPlugins[i].find("MiarmyExpress") >= 0:
            return True
...
def McdLicenseCheck():
    ...
    if McdIsLoadMiarmyExpress():
        allAgents = cmds.ls(type = "McdAgent")
        if len(allAgents) > 100:
            cmds.confirmDialog(t = "License Abort", m = "Miarmy Express can only render max upto 100 agents.\nGet Miarmy Pro for rendering more!")
            raise Exception("License Abort")



def McdRemoveBlankString(inStr):
    outStr = ""
