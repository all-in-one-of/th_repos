        ...
        if allChildren != None and allChildren != []:
            for i in range(len(allChildren)):
                if cmds.nodeType(allChildren[i]) == "McdAgentGroup":
                    ...
                    childOfChildNode = cmds.listRelatives(allChildren[i])
                    if childOfChildNode != None and childOfChildNode != []:
                        if cmds.nodeType(childOfChildNode[0]) == "McdAgentGroup":
                            cid = cmds.getAttr(childOfChildNode[0] + ".cid")
                            cidList.append(cid)
                            # ----------- referencing pipeline tracker : get agent name -------------------
                            if childOfChildNode[0].find(":") > 0:
                                name = childOfChildNode[0].split(":")[0]
                            else:
                                name = childOfChildNode[0].split("Agent_")[1]
                            agentList.append(name)
