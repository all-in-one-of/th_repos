...
def McdPlacementEditorGUI():
    ...
    cmds.rowColumnLayout(nc = 6, cw = [(1, 100), (2, 90), (3, 30), (4, 20), (5, 30), (6, 68)])
    for i in range(len(allAgents)):
        ...
        agentid = McdFromAgentName2ID(allAgents[i])
        
        # TEMP: MODIFIED
        activeAgnt = allAgents[i]
        ...
        if not len(optA)==0:
            cid = cmds.getAttr(optA[0]+".colorId") #index = cmds.setAttr(optA[0]+".colorId", index)
        if not len(optB)==0:
            cid = cmds.getAttr(optB[0]+".colorId") #index = cmds.setAttr(optB[0]+".colorId", index)
            
        drawColor = setColorOfDraw(cid)
        cmds.text(l = str(cid), bgc = drawColor)
        
        cmds.floatField("weightIF" + stri, cc = "changePPWeight(" + stri + ")")
    ...
    cmds.rowColumnLayout(nc = 4, cw = [(1, 80), (2, 80), (3, 110), (4, 40)])
    cmds.button(l = "Make Even", c = "makePlacementEven()")
    cmds.text(l = "")
    cmds.text(l = "")
    cmds.button(l = "Apply", c = "updatePPFromWeight()")
    cmds.text(l = "")
    cmds.text(l = "")
    cmds.text(l = "")
    cmds.button(l = "Clear", c = "clearPPWeightInfo()")
    cmds.setParent("..")
    ...
    cmds.rowColumnLayout(nc = 1, cw = [(1, 140)])
    cmds.button(l = "Set Type ID", c = "setIndiAgentType()")
    cmds.setParent("..")
