...
def setSingleNumericAttrSliderGrpMiG(option, control, nodeName, attrName):
    ...
    if option == 0:
        ...
        if control == "mgtbb_dfcst":
            if qValue == 1:
                cmds.floatFieldGrp("mbsst1_ccst", e = True, en = True)
            else:
                cmds.floatFieldGrp("mbsst1_ccst", e = True, en = False)
        ...
        if attrName == "boolMaster[10]" and qValue == 0:
            cmds.confirmDialog(t = "Warning", m = "Need reset, please put time at start time.")
...
#    folderName = cmds.textFieldGrp("mcc_cf", q = True, tx = True)
...
def MiarmyCacheChangeCacheName(globalNode):
    cacheName = cmds.textFieldGrp("mcc_cn", q = True, tx = True)
    
    realCacheName = envPath2AbsPath(cacheName)
    print "set name: " + realCacheName
...
def createMiddleRig(sameAsRig):
    # list all agent group
    # for each one:
    #   find MiddleRig group, if not create one
    #   walk through the oa, create new one
    #   set attr for t,r,s, orient, etc...
    #   create the custom attr
    
    oAgentResume()
    
    allAgtGrp = cmds.ls(type = "McdAgentGroup")
    if McdIsBlank(allAgtGrp):
        return
    
    for i in range(len(allAgtGrp)):
        
        allChild = cmds.listRelatives(allAgtGrp[i], c = True, p = False, path = True)
        ...
        for j in range(len(allChild)):
            if allChild[j].find("MiddleRig_") == 0:
                isDone = True
                break
        
        if isDone:
            print "remove old :" + allChild[j]
            cmds.delete(allChild[j])
        ...
        for j in range(len(allChild)):
            if allChild[j].find("OriginalAgent") >= 0:
                ...
                firstBone = mel.eval("McdSimpleCommand -exe 30;")
                if firstBone != "" and not MIsBlank(firstBone):
                    typeName = allChild[j].split("_")[-1]
                    dupNode = cmds.duplicate(allChild[j], name = "MiddleRig_" + typeName)
                    rootNode = dupNode[0]
                    
                    clearContent(rootNode, sameAsRig)
                    
                    # adding custom data
                    copyBones = mel.eval("McdSimpleCommand -exe 31;")
                    cmds.select(allChild[j])
                    orgBones = mel.eval("McdSimpleCommand -exe 31;")
                    
                    if MIsBlank(orgBones) or MIsBlank(copyBones):
                        continue
                    if len(orgBones) != len(copyBones):
                        continue
                    
                    
                    allAttrs = []
                    for k in range(len(orgBones)):
                        # get copy ud attr number
                        # clear all except
                        # add some attrs
                        try:
                            nbCustomData = cmds.getAttr(copyBones[k] + ".customDataNum")
                        except:
                            nbCustomData = 0
                            
                        rotOrder = cmds.getAttr(orgBones[k] + ".orgRotateOrder")
                        cmds.setAttr(copyBones[k] + ".rotateOrder", rotOrder)
                        
                        if k == 0:
                            allAttrs = cmds.listAttr(copyBones[k], ud = True)
                            
                        for l in range(len(allAttrs)):
                            if allAttrs[l].find("ateX") > 0 or \
                                        allAttrs[l].find("aleX") > 0 or \
                                        allAttrs[l].find("ateY") > 0 or \
                                        allAttrs[l].find("aleY") > 0 or \
                                        allAttrs[l].find("ateZ") > 0 or \
                                        allAttrs[l].find("aleZ") > 0:
                                continue

                            try:
                                cmds.deleteAttr(copyBones[k] + '.' + allAttrs[l])
                            except:
                                pass
                            
                        if nbCustomData > 0:
                            for l in range(nbCustomData):
                                strl = str(l)
                                cdName = cmds.getAttr(agtGrp + ".cdNames[" + str(cdCount) + "]")
                                cdCount += 1
                                if cdName != None and cdName != "":
                                    cmds.addAttr(copyBones[k], ln = cdName, at = "float", k = 1)
                                else:
                                    cmds.addAttr(copyBones[k], ln = "userData_" + strl, at = "float", k = 1)
                                
                                
                                
                                
                            
def clearContent(rootNode, sameAsRig):
...
#def exportFBX(nbFrame, rootJoint, startFrame):
#    
#    try:
...
#        msg += ' folder.'
#        cmds.confirmDialog(t = "FBX Python SDK Not Detected", m = msg)
#        return
#    
#    # Create the scene.
...
#
#    # Save the scene.
#    lResult = FbxCommon.SaveScene(lSdkManager, lScene, "d:/abc/color/test.fbx")
#    if lResult == False:
...
#    lSdkManager.Destroy()
#### C:\Program Files\Autodesk\FBX\FbxPythonSdk\2012.2\examples\Python\ExportScene01
