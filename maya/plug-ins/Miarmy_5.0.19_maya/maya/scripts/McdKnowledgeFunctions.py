...
def McdCreateRoad():
    ...
    try:
        ...
        try:
            ...
            cmds.confirmDialog(t = "Error", m = "Please convert it to road by Miarmy > Knowledge > Convert to Road Mode > Road Mode.")
...
def McdBuild2SidesRoad():
    fstRoad = getSelection("nurbsCurve")
    secRoad = getSelection2("nurbsCurve")
    
    # find connection to mcdroad!
    allDwNodes1 = McdGetAllDwLinks(fstRoad)
    allDwNodes2 = McdGetAllDwLinks(secRoad)
    
    if MIsBlank(allDwNodes1):
        cmds.confirmDialog(t = "Error", m = '1st selection is not road curve.')
        raise Exception("1st selection is not road curve")
    
    if cmds.nodeType(allDwNodes1[0]) != "McdRoad":
        cmds.confirmDialog(t = "Error", m = '1st selection is not road curve.')
        raise Exception("1st selection is not road curve")
    
    if MIsBlank(allDwNodes2):
        cmds.confirmDialog(t = "Error", m = '2nd selection is not road curve.')
        raise Exception("2nd selection is not road curve")
    
    if cmds.nodeType(allDwNodes2[0]) != "McdRoad":
        cmds.confirmDialog(t = "Error", m = '2nd selection is not road curve.')
        raise Exception("2nd selection is not road curve")
    ...
    if cmds.getAttr(road1 + ".sideConns") == 1:
        cmds.confirmDialog(t = "Error", m = 'already connected 1.')
        return
    
    if cmds.getAttr(road2 + ".sideConns") == 1:
        cmds.confirmDialog(t = "Error", m = 'already connected 2.')
        return
    
    try:
        cmds.connectAttr(road1 + ".sideConns", road2 + ".sideConns")
        cmds.setAttr(road1 + ".sideConns", 1)
    except:
        cmds.confirmDialog(t = "Error", m = 'already connected.')
...
def McdCreateGroupVar():
    ...
    # put the zone to the perception_set
    perceptionGrp = cmds.ls("AttrProxy_Set")
    if perceptionGrp == [] or perceptionGrp == None:
        #create one and parent
        cmds.group(n = "AttrProxy_Set", em = True)
        cmds.parent("AttrProxy_Set", "Miarmy_Contents")
    try:
        parentTemp = cmds.listRelatives("AttrProxy_Set", c = False, p = True)[0]
        if parentTemp != "Miarmy_Contents":
            cmds.parent("AttrProxy_Set", "Miarmy_Contents")
    except:
        pass
    try:
        cmds.parent(windNode, "AttrProxy_Set")
    except:
        pass
