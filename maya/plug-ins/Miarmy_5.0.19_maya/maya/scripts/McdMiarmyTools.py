...
def setupOmStateExp(value):
    cmds.menuItem(label = "arnold")
    cmds.menuItem(label = "alembic")
    
    if value == 0:
        cmds.optionMenu("ModeStat_om", e = True, v = "arnold")
    elif value == 1:
        cmds.optionMenu("ModeStat_om", e = True, v = "alembic")

def om_cc_Mode(control, nodeName, attrName):
    qValue = cmds.optionMenu("ModeStat_om", q = True, v = True)
...
def exportMiscUseCharCache(globalNode, brainNode):
    
    McdSetupExportFlagForSpeicalMeshes(True)
    
    qValue = cmds.optionMenu("ModeStat_om", q = True, v = True)
    print qValue
    if qValue == "arnold":
        exportShaveFur(globalNode, brainNode)
    elif qValue == "alembic":
        exportABC(globalNode, brainNode)
    
    McdSetupExportFlagForSpeicalMeshes(False)
        
def importMiscUseCharCache(globalNode, brainNode):
    qValue = cmds.optionMenu("ModeStat_om", q = True, v = True)
    if qValue == "arnold":
        importShaveFur(globalNode, brainNode)
    elif qValue == "alembic":
        importABC(globalNode, brainNode)
        
def exportShaveFur(globalNode, brainNode):
    
    dEnChar = cmds.getAttr(brainNode + ".enableChar")
    if not dEnChar:
        cmds.confirmDialog(t = "Abort", m = "Need make sure character cache OK and enable it.")
        return
        
    # getSelection("shaveHair")
    
    # get geo information:
    meshListRaw = []
    meshListRaw = mel.eval("McdGetRenderGeoCmd -rec 4;") # get and storing
    ...
    for i in range(len(meshListRaw)):
        if not isGetName:
            isGetName = True
        if meshListRaw[i] != "#":
            meshListUnit.append(meshListRaw[i])
        else:
            isGetName = False
    ...
    if MIsBlank(selObjs):
        cmds.confirmDialog(t = "Abort", m = "Please select some export contents, like ncloth, hair ect...")
        return
    
    selObjShapes = []
    for i in range(len(selObjs)):
        allShapes = cmds.listRelatives(selObjs[i], c = True, p = False)
        if MIsBlank(allShapes):
            selObjShapes.append("")
        else:
            realShape = ""
            for j in range(len(allShapes)):
                if not cmds.getAttr(allShapes[j] + ".intermediateObject"):
                    if cmds.nodeType(allShapes[j]) == "mesh":
                        realShape = allShapes[j]
                        break
            selObjShapes.append(realShape)
    ...
    totalAgent = dExpEndAgentID - dExpStartAgentID + 1
    startFrame = cmds.playbackOptions(q =True, min = True)
    endFrame = cmds.playbackOptions(q =True, max = True)
    
    stat = cmds.confirmDialog(t = "Export .ass", m = "The export information:\n" + \
                                            "Start Frame: " + str(startFrame) + "\n" + \
                                            "End Frame: " + str(endFrame) + "\n" + \
                                            "Start Agent ID: " + str(dExpStartAgentID) + "\n" + \
                                            "End Agent ID: " + str(dExpEndAgentID) + "\n" + \
    ...
    for i in range(totalAgent):
        ...
        # choose selection
        for j in range(len(selObjShapes)):
            if selObjShapes[j] == "":
                cmds.select(selObjs[j], tgl = 1)
                continue
            
            if selObjShapes[j] in meshList[aid-1]:
                cmds.select(selObjs[j], tgl = 1)


        # setup drive agent:
        cmds.setAttr(brainNode + ".charId", aid)
        cmd = 'arnoldExportAss -f "' + dExpFolder + '/' + dExpName + '.' + straid + '.ass" \
                    -startFrame ' + str(startFrame) + ' -s -endFrame ' + str(endFrame) + ' \
                    -mask 255 -lightLinks 1 -frameStep 1.0 -compressed \
                    -shadowLinks 1;'
        
        sels = cmds.ls(sl = True)
        if not MIsBlank(sels):
            mel.eval(cmd)
        
        
def importShaveFur(globalNode, brainNode):
    ...
    for i in range(totalAgent):
        ...
        nodeName = dExpName + straid;
        cmds.createNode("aiStandIn", n = nodeName)
    
        fillFile = dExpFolder + '/' + dExpName + '.' + straid + '.####.ass.gz'
...
def exportABC(globalNode, brainNode):
    ...
    stat = cmds.confirmDialog(t = "Export Alembic for Selections", m = "The export information:\n" + \
                                            "Start Frame: " + str(startFrame) + "\n" + \
                                            "End Frame: " + str(endFrame) + "\n" + \
                                            "Start Agent ID: " + str(dExpStartAgentID) + "\n" + \
                                            "End Agent ID: " + str(dExpEndAgentID) + "\n" + \
    ...
    for i in range(totalAgent):
        ...
        # setup drive agent:
        cmds.setAttr(brainNode + ".charId", aid)
        cmd = 'AbcExport -j "-frameRange ' + str(startFrame) + ' ' \
                + str(endFrame) + ' -uvWrite' + selObjsStr \
                + ' -file ' + dExpFolder + '/' + dExpName + '.' + straid + '.abc"'
        
        mel.eval(cmd)
        
        
def importABC(globalNode, brainNode):
    ...
    for i in range(totalAgent):
        ...
        fillFile = dExpFolder + '/' + dExpName + '.' + straid + '.abc'
        
        cmd = 'AbcImport -mode import "' + fillFile + '"'
...
def setSingleNumericAttrSliderGrpMiG(option, control, nodeName, attrName):
    ...
    if option == 0:
        ...
        qV = qValue.replace("\\", "/")
        cmds.setAttr(nodeName + "." + attrName, qV, type = "string")
...
def convertSCCache():
    ...
    if stat == "Yes":
        ...
        mel.eval("McdMakeJointCacheCmd -actionMode 4;")
...
def enableLayerCache(switch):
    
    if switch == 1:
        opt = cmds.confirmDialog(t = "Question", m = "Please choose the layer cache mode:" + \
                                                        "\n\n * Full Drive(==): simply drive agents by cache without further interactive." + \
                                                        "\n * As Input(>>): drive agents by cache and can continue read logic and enable dynamics\n" \
        
        , b = ["Full Drive", "As Input", "Cancel"])
        if opt == "Full Drive":
            switch = 1
        elif opt == "As Input":
            switch = 2
    ...
    globalNode = McdGetMcdGlobalNode()
    cmds.setAttr(globalNode + ".boolMaster[13]", 0)
    
    selPlaces = cmds.textScrollList("layercache_tsl", q = True, si = True)
    
    for j in range(len(selPlaces)):
        selPlace = selPlaces[j]
        if selPlace.find(" ") >= 0:
            selPlace = selPlace.split(" ")[-1]
        
        cmds.setAttr(selPlace + ".enableCache", switch)
        
        # re-draw list
        cmds.textScrollList("layercache_tsl", e = True, ra = True)
        ...
        cmds.textScrollList("layercache_tsl", e = True, append = allPlaces)
