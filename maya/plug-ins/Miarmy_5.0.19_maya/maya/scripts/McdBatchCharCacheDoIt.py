...
def McdBatchCharCacheDoIt():
    ...
    # place agent out (without progress bar)
    ...
    # make agent cache
    McdMakeCharCacheBatch()
    
    
def McdMakeCharCacheBatch():
    ...
    # make character cache -----------------------------------------------------
    cmds.setAttr(globalNode + ".enableCache", 1)
    solverFrame = cmds.getAttr(brainNode + ".startTime")
    cmds.currentTime(solverFrame)
    convertSCCacheBatch()
...
def convertSCCacheBatch():
    ...
    if not os.access(cacheFolder, os.W_OK):
        print cacheFolder
        raise Exception("The character cache output folder is not writable.")
