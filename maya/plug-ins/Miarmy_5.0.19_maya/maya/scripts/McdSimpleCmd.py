...
def McdSelectMcdGlobal(showMsg = True):
...
def McdCreateStateCmd():
    ...
    if newState != "":
        ...
        if not isReferenceScene():
            ...
            activeAgentName = McdGetActiveAgentName()
            McdCreateStateReference(newState, activeAgentName)
...
def McdCreateActionShellCmd():
    ...
    if not isReferenceScene():
        ...
        McdCreateAction(activeAgentName)
    
    else:
        activeAgentName = McdGetActiveAgentName()
        McdCreateActionReference(activeAgentName)
...
def cctSwitch():
    try:
        recordSel = cmds.ls(sl = True)
        selObj = cmds.ls(type = "McdAgent")[0]
        cmds.getAttr(selObj + "._SwitchCCTVis")
        if recordSel != [] and recordSel != None:
            cmds.select(clear = True)
            cmds.select(recordSel)
        else:
...
def McdReturnClothPoints():
    ...
    if not MIsBlank(allActions):
        ...
        for i in range(len(allActions)):
            nbGeo = cmds.getAttr(allActions[i] + ".nbGeo")
            if nbGeo > 0:
                for j in range(nbGeo):
                    strj = str(j)
                    geoName = cmds.getAttr(allActions[i] + ".geoNames[" + strj + "]")
                    
                    geoNode = cmds.ls(geoName)
                    
                    if not MIsBlank(geoNode):
                        
                        geoObj = geoNode[0]
                        
                        if cmds.nodeType(geoObj) == "mesh":
                    
                            nbPoints = cmds.polyEvaluate(geoObj, v = True)
                    
                            for k in range(nbPoints):
                                strk = str(k)
                                try:
                                    cmds.setAttr(geoName + ".pt[" + strk + "].px", 0)
                                    cmds.setAttr(geoName + ".pt[" + strk + "].py", 0)
                                    cmds.setAttr(geoName + ".pt[" + strk + "].pz", 0)
                                except:
                                    pass
...
def McdAddDeformingAttr():
    selObj = cmds.ls(sl = True)
    if selObj != None and selObj != []:
        for i in range(len(selObj)):
            try:
                cmds.addAttr(selObj[i], ln = "deforming", at = "long", dv = 1)
                cmds.setAttr(selObj[i] + ".deforming", k = False, cb = True)
            except:
                pass
            try:
                cmds.setAttr(selObj[i] + ".deforming", 1)
            except:
                pass
            ...
            if not MIsBlank(allChildren):
                try:
                    cmds.addAttr(allChildren[0], ln = "deforming", at = "long", dv = 1)
                    cmds.setAttr(allChildren[0] + ".deforming", k = False, cb = True)
                except:
                    pass
                try:
                    cmds.setAttr(allChildren[0] + ".deforming", 1)
                except:
                    pass
    
    
def McdAddLandTargetIDAttr():
    selObj = cmds.ls(sl = True)
    if selObj != None and selObj != []:
        for i in range(len(selObj)):
            ...
            allChildren = cmds.listRelatives(selObj[i], c = True, p = False, path = True)
            if not MIsBlank(allChildren):
                if cmds.nodeType(allChildren[0]) == "locator":
            
                    try:
                        cmds.addAttr(selObj[i], ln = "landID", at = "long", dv = 1)
                        cmds.setAttr(selObj[i] + ".landID", k = False, cb = True)
                    except:
                        pass
                    try:
                        cmds.setAttr(selObj[i] + ".landID", 1)
                    except:
                        pass
                else:
                    cmds.confirmDialog(t = "Warning", m = "You need to select a locator and try again.")
                    
            
def McdAddMeshAttachIDAttr():
    selObj = cmds.ls(sl = True)
    if selObj != None and selObj != []:
        for i in range(len(selObj)):
            ...
            if not MIsBlank(allChildren):
                if cmds.nodeType(allChildren[0]) == "mesh":
                    ...
                    try:
                        cmds.addAttr(selObj[i], ln = "attachID", at = "long", dv = 1)
                        cmds.setAttr(selObj[i] + ".attachID", k = False, cb = True)
                    except:
                        pass
                    try:
                        cmds.setAttr(selObj[i] + ".attachID", 1)
                    except:
                        pass
                else:
                    cmds.confirmDialog(t = "Warning", m = "You need to select a polygon mesh and try again.")

                
    
def McdDeleteDeformingAttr():
    selObj = cmds.ls(sl = True)
    if selObj != None and selObj != []:
        for i in range(len(selObj)):
            try:
                cmds.setAttr(selObj[i] + ".deforming", 0)
            except:
                pass
            
            allChildren = cmds.listRelatives(selObj[i], c = True, p = False)
            if not MIsBlank(allChildren):
                try:
                    cmds.setAttr(allChildren[0] + ".deforming", 0)
                except:
                    pass
...
def createPxKineBoxSels():

    # get selection:
    allTransRaw = cmds.ls(sl = True, type = "transform")
    
    if MIsBlank(allTransRaw):
        raise Exception('Nothing selected.')
    
    allTrans = []
    allShapes = []
    for i in range(len(allTransRaw)):
        try:
            child = cmds.listRelatives(allTransRaw[i], c = True, p = False)[0]
            if cmds.nodeType(child) == "mesh":
                allTrans.append(allTransRaw[i])
                allShapes.append(child)
        except:
            pass    
    ...
    for i in range(len(allTrans)):
        ...
        nbNode = str(McdGetNumOfThisType("McdKinePrim"))
        newNodeTrans = cmds.createNode("transform", n = "McdKinePrim" + nbNode)
        newNode = cmds.createNode("McdKinePrim", n = "McdKinePrim" + nbNode + "Shape", p = newNodeTrans)
        
        cmds.parent(newNodeTrans, allTrans[i])
        
        # match the bbx:
        cmds.setAttr(newNodeTrans + ".rx", 0)
        cmds.setAttr(newNodeTrans + ".ry", 0)
        cmds.setAttr(newNodeTrans + ".rz", 0)
        
        bbnx = cmds.getAttr(allShapes[i] + ".bbnx")
        bbny = cmds.getAttr(allShapes[i] + ".bbny")
        bbnz = cmds.getAttr(allShapes[i] + ".bbnz")
        
        bbxx = cmds.getAttr(allShapes[i] + ".bbxx")
        bbxy = cmds.getAttr(allShapes[i] + ".bbxy")
        bbxz = cmds.getAttr(allShapes[i] + ".bbxz")
        
        cmds.setAttr(newNodeTrans + ".tx", 0.5 * (bbnx + bbxx))
        cmds.setAttr(newNodeTrans + ".ty", 0.5 * (bbny + bbxy))
        cmds.setAttr(newNodeTrans + ".tz", 0.5 * (bbnz + bbxz))
        
        cmds.setAttr(newNodeTrans + ".sx", bbxx - bbnx)
        cmds.setAttr(newNodeTrans + ".sy", bbxy - bbny)
        cmds.setAttr(newNodeTrans + ".sz", bbxz - bbnz)
...
def createMarkBuoyancyMesh():
    ...
    newNode = cmds.ls(sl = True)
    if newNode != [] or newNode != None:
        cmds.setAttr(newNode[0] + ".primType", 3)
...
def McdExportAgent():
    ...
    # Remove any bindPose connection to Miarmy_Contents
    bind_pose_connections = []
    if cmds.objExists('Miarmy_Contents'):
        bind_pose_connections = cmds.listConnections('Miarmy_Contents', type='dagPose', plugs=True)
        if bind_pose_connections:
            for bind_pose_connection in bind_pose_connections:
                cmds.disconnectAttr('Miarmy_Contents.message', bind_pose_connection)
    ...
    # Restore bindPose connections
    if bind_pose_connections:
        for bind_pose_connection in bind_pose_connections:
            cmds.connectAttr('Miarmy_Contents.message', bind_pose_connection, force=True)
...
def checkTextureForPP():
    pass 
    
    
    
def McdAddRTDisplayFlag():
    ...
    for i in range(len(selObj)):
        children = cmds.listRelatives(selObj[i], c = True, p = False)
        if MIsBlank(children):
            continue
        ...
        for j in range(len(children)):
            ...
            try:
                cmds.addAttr(child, ln = "McdRTDisplay", at = "bool", dv = 1)
                cmds.setAttr(child + ".McdRTDisplay", k = False, cb = True)
            except:
                pass
            
            try:
                cmds.addAttr(child, ln = "McdColorID", at = "long", dv = 1)
                cmds.setAttr(child + ".McdColorID", k = False, cb = True)
            except:
                pass
    
def McdMarkRepalBoundMesh():
    selObj = cmds.ls(sl = True)
    if MIsBlank(selObj):
        return
    ...
    for i in range(len(selObj)):
        try:
            ...
            if (cmds.nodeType(childNode) == "mesh"):
                cmds.addAttr(childNode, ln = "autoRepelDistance", at = "double")
                cmds.setAttr(childNode + ".autoRepelDistance", k = False, cb = True)
        except:
            cmds.confirmDialog(t = "Error", m = "Cannot add auto repal distance, or already added.")
    
    
def McdGetAgentGeoList(agentNumber = 10):
    nbAgent = 100000 + agentNumber
    melCmd = "McdAgentMatchCmd -mm " + str(nbAgent)
    
    resultStr = mel.eval(melCmd)
    
    return resultStr
    

def McdVRSceneRearrange():
    # read render settings and check:
    
    #vraySettings.vrscene_render_on
    #vraySettings.misc_separateFiles
    #vraySettings.misc_eachFrameInFile
    #vraySettings.misc_compressedVrscene
    #vraySettings.vrscene_filename
    #defaultRenderGlobals.animation
    
    renderGlobalTemp = cmds.ls(type = "renderGlobals")
    if MIsBlank(renderGlobalTemp):
        cmds.confirmDialog(t = "Error", m = "Cannot find Maya render global node")
        return
    renderGlobal = renderGlobalTemp[0]
    
    vrRenderGlobalTemp = cmds.ls(type = "VRaySettingsNode")
    if MIsBlank(vrRenderGlobalTemp):
        cmds.confirmDialog(t = "Error", m = "Cannot find V-Ray render global node")
        return
    vrRenderGlobal = vrRenderGlobalTemp[0]
    
    vrInfoHost = cmds.ls("McdVRayInfoHost")
    if MIsBlank(vrInfoHost):
        cmds.confirmDialog(t = "Error", m = "Cannot find V-Ray Info Host Node")
        return
    vrInfoHostNode = vrInfoHost[0]
    vrFolder = cmds.getAttr(vrInfoHostNode + ".outputVRFolder")
    vrName = cmds.getAttr(vrInfoHostNode + ".outputVRName")
    

    renderOn = cmds.getAttr(vrRenderGlobal + ".vrscene_render_on")
    if renderOn:
        cmds.confirmDialog(t = "Warning", m = "Please turn off render.")
        return
    
    setting = cmds.getAttr(vrRenderGlobal + ".misc_separateFiles")
    if not setting:
        cmds.confirmDialog(t = "Warning", m = "Please export .vrscene as seperated files.")
        return
    
    setting = cmds.getAttr(vrRenderGlobal + ".misc_eachFrameInFile")
    if setting:
        cmds.confirmDialog(t = "Warning", m = "Please disable each frame and export as a single file")
        return
    
    setting = cmds.getAttr(vrRenderGlobal + ".misc_compressedVrscene")
    if setting:
        cmds.confirmDialog(t = "Warning", m = "Please do not export as compressed vrscene")
        return
    
    try:
        setting = cmds.getAttr(renderGlobal + ".animation")
        if not setting:
            cmds.confirmDialog(t = "Warning", m = "Please export as animation (not single frame)")
            return
    except:
        pass
    
    # get path:
    fileName = cmds.getAttr(vrRenderGlobal + ".vrscene_filename")
    
    if fileName.find(".vrscene") < 0:
        fileName += ".vrscene"
    ...
    exportName = dirName.split("/")[-1]
    geoExportName = "geo" + exportName
    
    print geoExportName
    
    baseNamePre = baseName.split(".vr")[0]
    print baseNamePre
    
    if not os.access(fileName, os.W_OK):
        cmds.confirmDialog(t = "Warning", m = fileName + "\nAbove .vrscene file not writable")
        return
    
    # frame range
    startFrame = int(cmds.getAttr(renderGlobal + ".startFrame") + 0.01)
    endFrame = int(cmds.getAttr(renderGlobal + ".endFrame") + 0.01)
    
    print startFrame
    print endFrame
    
    # create folder
    if not os.access(dirName + "/frames_geometry", os.R_OK):
        os.mkdir(dirName + "/frames_geometry")
        
    geoFile = baseNamePre + "_geometry.vrscene"
    nbFrame = endFrame - startFrame + 1
    
    geoFileMain= "_main_" + geoFile
    f_main= open(dirName + "/frames_geometry/" + geoFileMain, "w")
    mainStr = ""
    
    for i in range(nbFrame):
        currentFrame = startFrame + i
        frameStrRaw = str(currentFrame)
        ...
        while(1):
            if len(frameStr) < 4:
                ...
                break
        geoFileNew = baseNamePre + "_geometry." + frameStr + ".vrscene"
        
        
        
        # get all bounding box of current frame:
        bbxStrList = []
        geoFileExport = vrFolder + "/" + vrName + "/geo" + vrName + "." + frameStr + ".txt"
        f_ref = open(geoFileExport, "r")
        
        while True:
            geoLine = f_ref.readline()
            if geoLine:
                if geoLine.find('box "') > 0:
                    bboxInfo = geoLine.split("box ")[-1]
                    bboxInfoStr = ""
                    if bboxInfo.find("\r\n") > 0:
                        bboxInfoStr = bboxInfo.split("\r\n")[0]
                    elif bboxInfo.find("\n") > 0:
                        bboxInfoStr = bboxInfo.split("\n")[0]
                    else:
                        bboxInfoStr = bboxInfo
                    bbxStrList.append(bboxInfoStr)
            else:
                break
        f_ref.close()
        ...
        # modify file
        bbxCounter = 0;
        bbxMax = len(bbxStrList) - 1
        if i == 0:
            print bbxMax
        f_in = open(dirName + "/" + geoFile, "r")
        f_out = open(dirName + "/frames_geometry/" + geoFileNew, "w")
        while True:
            lineString = f_in.readline()
            if lineString:
                if lineString.find("McdVRGeom") >= 0:
                    f_out.write(lineString)
                    readCounter = 13
                    while True:
                        lineString = f_in.readline()
                        if lineString:
                            if lineString.find("currentFrame=") > 0:
                                lineStringNew = "  currentFrame=interpolate(("+ frameStrRaw +","+ frameStrRaw +"));\n"
                                f_out.write(lineStringNew)
                            elif lineString.find("bbox=") > 0:
                                lineStringNew = "  bbox=interpolate((" + frameStrRaw + "," + bbxStrList[bbxCounter] + "));\n"
                                if bbxCounter < bbxMax:
                                    bbxCounter += 1
                                f_out.write(lineStringNew)
                            else:
                                f_out.write(lineString)
        ...
        f_in.close()
        f_out.close()
        mainStr += "#include \"" + dirName + "/frames_geometry/" + geoFileNew + "\"\n\n"
        

    f_main.write(mainStr)
    f_main.close()
    ...
    if firstLine.find("#include") < 0:
        f.seek(0)
        f.write("#include \"" + dirName + "/frames_geometry/" + geoFileMain + "\"\n//");
        
    f.close()


def McdPreCheckAndMarkActions():
    
    ###########
    # need upgrade
    globalNode = McdGetMcdGlobalNode();
    if cmds.getAttr(globalNode + ".boolMaster[17]") == 1:
        print "already marked to new miarmy."
        return
    cmds.setAttr(globalNode + ".boolMaster[17]", 1)
    
    # get all states: states1 and states2
            ...
            for j in range(len(allStates)):
                for k in range(len(allStates1)):
                    ...
                    if not MIsBlank(allTransAct):
                        try:
                            cmds.setAttr(allTransAct[0] + ".active", 1)
                        except:
