...
def McdARSetupCurrentFrame():
    ...
    #stat = cmds.confirmDialog(t = "Random Texture", m = "Do you want to randomize your texture?", b = ["Yes", "Canncel"])
    #if stat == "Yes":
    duplicatedObject = RenderFBX.McdRandomizeTexturesDSO("arnold", shaderFile)
    printCurrentShaderInfo()
    ...
    RenderFBX.clearUselessObjAndShaders(duplicatedObject)
    
def printCurrentShaderInfo():
    ...
    print ""
    print "*********************************************************************"
    print "************     Print Current Shader Info in Scene     *************"
    print "*"
    allHost = allGeos
    if MIsBlank(allHost):
        print "* No randomize"
    ...
    for i in range(len(allHost)):
        if cmds.nodeType(allHost[i]) != "mesh":
            continue
        print "* Shader Holder: " + allHost[i]
        
        allConns = cmds.listConnections(allHost[i], d = True, s = False)
        if MIsBlank(allConns):
            print "* Shading Engine: None"
            continue
        sgNode = ""
        ...
        print "* Shading Engine: " + sgNode
        
        print "* Var SG: "
        
        allConns = cmds.listConnections(sgNode, s = False, d = True)
        if MIsBlank(allConns):
            print "*     None"
            ...
            for j in range(len(allConns)):
                if cmds.nodeType(allConns[j]) == "shadingEngine":
                    ...
                    allHis = cmds.listHistory(allConns[j])
                    for k in range(len(allHis)):
                        if cmds.nodeType(allHis[k]) == "file" or cmds.nodeType(allHis[k]) == "aiImage":
                            
                            if cmds.nodeType(allHis[k]) == "file":
                                texTemp = cmds.getAttr(allHis[k] + ".fileTextureName")
                                
                                allConns2 = cmds.listConnections(allHis[k], s = False, d = True, p = True)
                                if not MIsBlank(allConns2):
                                    for n in range(len(allConns2)):
                                        if allConns2[n].find(".KsColor") > 0 or allConns2[n].find(".color") > 0 or allConns2[n].find(".displacement") > 0:
                                            nodeName = allConns2[n].split(".")[0]
                                            print "*          shader: " + nodeName
                                
                            else:
                                texTemp = cmds.getAttr(allHis[k] + ".filename")
                            ...
                            print "*           -> imagePath: " + allHis[k] + " >>> " + texTemp
...
# all: McdARSetupAllFrame_Batch(1001, 1001, True)

# frame range : McdARSetupAllFrame_Batch(1001, 1020, False)
# frame range : McdARSetupAllFrame_Batch(1021, 1040, False)
# frame range : McdARSetupAllFrame_Batch(1041, 1060, False)

def McdARSetupAllFrame_Batch(minFrame, maxFrame, exportPreContents = True):
    ...
    duplicatedObject = RenderFBX.McdRandomizeTexturesDSO("arnold", shaderFile, exportPreContents)
...
def McdDeleteShaderTrashNodes():
    
    allSGNodes = cmds.ls(type = "shadingEngine")
    
    for i in range(len(allSGNodes)):
        if allSGNodes[i].find("__mcd") > 0:
            cmds.delete(allSGNodes[i])
            print "delete: " + allSGNodes[i]
