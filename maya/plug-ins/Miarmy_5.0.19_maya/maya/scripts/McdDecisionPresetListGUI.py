...
def McdDecisionPresetListGUI(sentenceID, dcNode):
    ...
    cmds.text(l = "Decision", fn = "smallBoldLabelFont", align = "left", width = 250)
    
    decisionPresetList = []
    
    decisionPresetList = appendDecisionTypes()
    
    
    cmds.textScrollList("mcddol_tsltype", numberOfRows = len(decisionPresetList), nr = 32, append = decisionPresetList, \
                                    sc = 'McdUpdateSubDecisionDetail(' + str(sentenceID) + ', "' + dcNode + '")')
        
    cmds.textScrollList("mcddol_tsl", dcc = 'McdAutoFillDecisionDetail(' + str(sentenceID) + ', "' + dcNode + '")')
...
def appendDecisionTypes():
    decisionPresetList = []
    decisionPresetList.append("Move")
    decisionPresetList.append("HP & MP")
    decisionPresetList.append("Disable Detection")
    decisionPresetList.append("Physics")

    return decisionPresetList

def appendDecisions(txt):
    
    decisions = []
    
    if txt == "Move":
        decisions.append("Move forward as speed 20 (tz)(Unit/Sec)")
        decisions.append("Turn left as speed 120(ry)(Degree/Sec)")
        decisions.append("Turn right as speed 120 (ry)(Degree/Sec)")
        decisions.append("Go up as speed 20 (ty)(Unit/Sec)")
        decisions.append("Go down as speed 20(ty)(Unit/Sec)")
    
    elif txt == "HP & MP":
        decisions.append("My hit point is dropping 2 per sec (ry)(Unit/Sec)")
        decisions.append("My mana point is dropping 2 per sec (ry)(Unit/Sec)")
    
    elif txt == "Disable Detection":
        decisions.append("Block sound of agent (sound.mute)")
        decisions.append("Block vision of agent (vision.mute)")
    
    elif txt == "Physics":
        decisions.append("Turn me into ragdoll")
        decisions.append("Turn me into ragdoll and push")
        decisions.append("Give my ragdoll servo force")
        decisions.append("Enable partial ragdoll from my bone: XXX")
        decisions.append("Brake and enable ragdoll from my bone: XXX")
        
    return decisions
    

def McdUpdateSubDecisionDetail(sentenceID, dcNode):
    choiceText = cmds.textScrollList("mcddol_tsltype", q = True, si = True)[0]
    
    decisionPresetList = []
    
    decisionPresetList = appendDecisions(choiceText)
    
    print decisionPresetList
    
    cmds.textScrollList("mcddol_tsl", e = True, ra = True)
    cmds.textScrollList("mcddol_tsl", e = True, append = decisionPresetList)
