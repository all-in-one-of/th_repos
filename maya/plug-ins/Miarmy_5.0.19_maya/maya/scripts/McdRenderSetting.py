...
def setStringAttrGrp(isPath, control, nodeName, attrName):
    ...
    if control == "rsar_ppp":
        if ((not os.access(qValue + "/McdArnoldDSO2.dll", os.R_OK)) and (not os.access(qValue + "/McdArnoldDSO2.so" , os.R_OK)) and (not os.access(qValue + "/McdArnoldDSO2.dylib" , os.R_OK)) ):
            cmds.confirmDialog(t = "Error", m = "Can not find McdArnoldDSO2 in the path you filled, will cause error when render.")
            qValue = ""
...
def McdConvertAssToAgentIDPass(sumIDs = 3):
    
    folderName = cmds.textFieldGrp("rsar_fld", q = True, tx = True)
    fileName = cmds.textFieldGrp("rsar_omn", q = True, tx = True)
    assFolder = folderName + "/" + fileName + "/assStandin/"
    newFolder = assFolder + "/AgentIDAss"
    
    #list all files
    allAss = os.listdir(assFolder)
    if len(allAss) <= 0:
        return
    
    allFiles = []
    allBBXFiles = []
    for i in range(len(allAss)):
        if allAss[i].find(".ass") > 0 and allAss[i].find(".asstoc") < 0:
            allFiles.append(allAss[i])
        if allAss[i].find(".asstoc") > 0:
            allBBXFiles.append(allAss[i])
    
    if len(allFiles) <= 0:
        return    
    
    #create new folder
    if not os.access(newFolder, os.R_OK):
        os.mkdir(newFolder)
        print "folder created: " + newFolder
        
    if not os.access(newFolder, os.R_OK):
        raise Exception("access denied: " + newFolder)
    
    for i in range(len(allBBXFiles)):    
        shutil.copy(assFolder + "/" + allBBXFiles[i], newFolder + "/" + allBBXFiles[i])
    
    for i in range(len(allFiles)):
        newFileContents = ""
        
        #read and find and replace    
        in_f = open(assFolder + "/" + allFiles[i], "r")
        allMatNames = []
        currentAgentNum = -1
        while(1):
            lineContents = in_f.readline()
            if lineContents:
                if lineContents.find("G_agentNum") >= 0:
                    agentNum = lineContents.split("G_agentNum ")[-1]
                    currentAgentNum = int(agentNum)

                if lineContents.find(" shader ") == 0:
                    idStr = str(currentAgentNum % sumIDs)
                    newLine = " shader surfaceShaderAOV" + idStr
        ...
        shaderStringR = "MayaShadingEngine\n" + \
                            "{\n" + \
                            " name surfaceShaderAOV0\n" + \
                            " beauty mcdss0\n" + \
                            "}\n" + \
                            "MayaSurfaceShader\n" + \
                            "{\n" + \
        ...
        shaderStringG = "MayaShadingEngine\n" + \
                            "{\n" + \
                            " name surfaceShaderAOV1\n" + \
                            " beauty mcdss1\n" + \
                            "}\n" + \
                            "MayaSurfaceShader\n" + \
                            "{\n" + \
        ...
        shaderStringB = "MayaShadingEngine\n" + \
                            "{\n" + \
                            " name surfaceShaderAOV2\n" + \
                            " beauty mcdss2\n" + \
                            "}\n" + \
                            "MayaSurfaceShader\n" + \
                            "{\n" + \
        ...
        # write to new one:
        newFileContents += shaderStringR
        newFileContents += shaderStringG
        newFileContents += shaderStringB
        
        # writing:
        f = open(newFolder + "/" + allFiles[i], "w")
