...
def CreateAndUpdateFootLoc(isFirstFrame = False):
    ...
    for i in range(feetNum):
        ...
        if isFirstFrame:
            ...
            currentLoc = cmds.ls(locatorName)
            if currentLoc == None or currentLoc == []:
                cmds.spaceLocator(n = locatorName)
                cmds.addAttr(locatorName, ln = "typeID", at = "long")
                cmds.setAttr(locatorName + ".typeID", k = False, cb = True)
        
        cmds.setAttr(locatorName + ".v", returnVal[i * 11])
        cmds.setAttr(locatorName + ".t", returnVal[i * 11 + 1], returnVal[i * 11 + 2], returnVal[i * 11 + 3])
        cmds.setAttr(locatorName + ".r", returnVal[i * 11 + 4], returnVal[i * 11 + 5], returnVal[i * 11 + 6])
        cmds.setAttr(locatorName + ".s", returnVal[i * 11 + 7], returnVal[i * 11 + 8], returnVal[i * 11 + 9])
        cmds.setAttr(locatorName + ".typeID", returnVal[i * 11 + 10])
        
        if isFirstFrame:
            cmds.parent(locatorName, "McdFootLocatorGroup")

        #keyframe
        rrr = cmds.setKeyframe(locatorName, at = "t")
        rrr = cmds.setKeyframe(locatorName, at = "r")
        rrr = cmds.setKeyframe(locatorName, at = "v")
