...
def McdRefreshWindows():
    ...
    globalNode = McdGetMcdGlobalNodeRW()
    if globalNode == None:
        return
    disableRefresh = cmds.getAttr(globalNode + ".boolMaster[26]")
    if disableRefresh:
        return
    ...
    try:
        if selType == "McdState" or selType == "McdActionShell":
            
            if selType == "McdActionShell":
                if not cmds.getAttr(selObj + ".active"):
                    return
