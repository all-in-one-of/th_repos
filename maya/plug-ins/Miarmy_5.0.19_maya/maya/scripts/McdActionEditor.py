...
def setSingleNumericAttr(option, control, nodeName, attrName):
    ...
    if option == 0:
        ...
        elif control == "actMNA_cb":
            cmds.setAttr(nodeName + ".isCached", False)
            edttrigger = cmds.getAttr(nodeName + "._ClearActionMsg")
        ...
        if attrName == "entryMax":
            ...
            if qValue > (80):
                stat = cmds.confirmDialog(t = "Warning", m = "This value should not be too big, you can set it but it better be less than: " \
                                        + "80%", b = ["Set anyway", "Cancel"])
                if stat == "Cancel":
...
def setPercentNumericAttr(option, control, nodeName, attrName, dLength):
    ...
    if option == 1:
        ...
        if control == "actCyF_ff":
            if qValue > 50:
                qValue = 50
                cmds.floatField(control, e= True, v = 50)
...
def setExitPAction(index, nodeName):
    stri = str(index)
    exitName = cmds.textField("actPExit" + stri, q = True, tx = True)
    exitNamePrune = exitName.split(" ")[0]
    cmds.setAttr(nodeName + ".exitPChoices[" + stri + "]", exitNamePrune, type = "string")
...
def parsePhaseData(actNode):
    ...
    melCmd = "McdSetIKDataCmd -om 1;"
    if mel.eval(melCmd):
        cmds.text("phaseDataAva", e = True, l = "available")
        
        dPeriod = cmds.getAttr(actNode + ".period")
        cmds.intField("periodIF", v = dPeriod, e = True)
        
    else:
        cmds.confirmDialog(t = "Create Failure", m = "Please fill the target bone name need parse.")
        raise
        
    nbFrame = cmds.getAttr(actNode + ".length")
    
    # check connection
    allConns = cmds.listConnections(actNode + ".visibility", d = True, s = False)
    psrNode = ""
    if MIsBlank(allConns):
        # if not create and connect
        psrNode = cmds.createNode("polySplitRing", ss = True)
        cmds.connectAttr(dcNode + ".visibility", psrNode + ".worldSpace")
        psrNode2 = cmds.createNode("polySplitRing", ss = True)
        cmds.connectAttr(dcNode + ".visibility", psrNode2 + ".worldSpace")
    else:
        psrNode = allConns[0]
        psrNode2 = allConns[1]
    ...
    for i in range(nbFrame):
        phase = cmds.getAttr(actNode + ".phaseData[" + str(i) + "]")
        phaseData.append(phase);
        
        diff = cmds.getAttr(actNode + ".diffData[" + str(i) + "]")
    ...
    nbFramef = float(nbFrame)
    for i in range(50):
        graph = float(i) / 49.0
        ratio = float(i) / 50.0
        frame = int(ratio * nbFramef)
        phase = phaseData[frame]
        diff = 0.0
        if abs(diffSum) > 0.0:
            diff = (diffData[frame] - diffMin) / diffSum
        
        
        cmds.setAttr( psrNode + ".profileCurve[" + str(i) + "].profileCurve_Interp", 1)
        cmds.setAttr( psrNode + ".profileCurve[" + str(i) + "].profileCurve_Position", graph)
        cmds.setAttr( psrNode + ".profileCurve[" + str(i) + "].profileCurve_FloatValue", phase)
        
    
        cmds.setAttr( psrNode2 + ".profileCurve[" + str(i) + "].profileCurve_Interp", 1)
        cmds.setAttr( psrNode2 + ".profileCurve[" + str(i) + "].profileCurve_Position", graph)
        cmds.setAttr( psrNode2 + ".profileCurve[" + str(i) + "].profileCurve_FloatValue", diff)
    

def updatePhaseDataView(actNode):
    ...
    for i in range(50):
        ...
        cmds.setAttr( psrNode2 + ".profileCurve[" + str(i) + "].profileCurve_Interp", 1)
        cmds.setAttr( psrNode2 + ".profileCurve[" + str(i) + "].profileCurve_Position", graph)
        cmds.setAttr( psrNode2 + ".profileCurve[" + str(i) + "].profileCurve_FloatValue", 0)

def setPhaseData1(actNode):
    # setup:
    hRatio = 0
    stat = cmds.promptDialog(t = "Set Phase 1", m = "Please fill the start frame ", \
                    button = ["OK", "Cancel"], \
                    defaultButton = "OK", cancelButton = "Cancel", dismissString = "Cancel")
    if stat == "OK":
        hRatio = int(cmds.promptDialog(query = True, text = True))
    if stat == "Cancel":
        return;
    
    
    wRatio = 0
    stat = cmds.promptDialog(t = "Set Phase 1", m = "Please fill the end frame ", \
                    button = ["OK", "Cancel"], \
                    defaultButton = "OK", cancelButton = "Cancel", dismissString = "Cancel")
    if stat == "OK":
        wRatio = int(cmds.promptDialog(query = True, text = True))
    if stat == "Cancel":
        return;
    
    nbFrame = wRatio - hRatio
    for i in range(nbFrame+1):
        stri = str(hRatio + i)
        cmds.setAttr(actNode + ".phaseData[" + stri + "]", 1.0)
        
    
    updatePhaseDataView(actNode)


def setPhaseData0(actNode):
    # setup:
    hRatio = 0
    stat = cmds.promptDialog(t = "Set Phase 0", m = "Please fill the start frame ", \
                    button = ["OK", "Cancel"], \
                    defaultButton = "OK", cancelButton = "Cancel", dismissString = "Cancel")
    if stat == "OK":
        hRatio = int(cmds.promptDialog(query = True, text = True))
    ...
    wRatio = 0
    stat = cmds.promptDialog(t = "Set Phase 0", m = "Please fill the end frame ", \
                    button = ["OK", "Cancel"], \
                    defaultButton = "OK", cancelButton = "Cancel", dismissString = "Cancel")
    if stat == "OK":
        wRatio = int(cmds.promptDialog(query = True, text = True))
    ...
    for i in range(nbFrame+1):
        stri = str(hRatio + i)
        cmds.setAttr(actNode + ".phaseData[" + stri + "]", 0.0)
        
    
    updatePhaseDataView(actNode)


def setPhaseDataSine(actNode):
    # setup:
    hRatio = 0
    stat = cmds.promptDialog(t = "Set Phase Sine", m = "Please fill the start frame ", \
                    button = ["OK", "Cancel"], \
                    defaultButton = "OK", cancelButton = "Cancel", dismissString = "Cancel")
    if stat == "OK":
        hRatio = int(cmds.promptDialog(query = True, text = True))
    ...
    wRatio = 0
    stat = cmds.promptDialog(t = "Set Phase Sine", m = "Please fill the end frame ", \
                    button = ["OK", "Cancel"], \
                    defaultButton = "OK", cancelButton = "Cancel", dismissString = "Cancel")
    if stat == "OK":
        wRatio = int(cmds.promptDialog(query = True, text = True))
    ...
    freq = 0.0
    stat = cmds.promptDialog(t = "Set Phase Sine", m = "Please fill the sine function frequence ", \
                    button = ["OK", "Cancel"], \
                    defaultButton = "OK", cancelButton = "Cancel", dismissString = "Cancel")
    if stat == "OK":
        freq = float(cmds.promptDialog(query = True, text = True))
    if stat == "Cancel":
        return;
    ...
    for i in range(nbFrame+1):
        stri = str(hRatio + i)
        cmds.setAttr(actNode + ".phaseData[" + stri + "]", (math.sin(float(i)*freq) + 1.0) * 0.5)
...
def initIKLockSlot(actNode, i, dLength):
    ...
    cmds.floatField("ik_li1" + stri, v = dIKLockIn1, max = dLength, \
                cc = 'setIKLockAttr(' + stri + ', "ik_li1", "'+actNode+'", "IKLockIns1Root")', vis = False)
    ...
    cmds.floatField("ik_lo2" + stri, v = dIKLockOut2, max = dLength, \
                cc = 'setIKLockAttr(' + stri + ', "ik_lo2", "'+actNode+'", "IKLockOuts2Root")', vis = False)
    
    cmds.text(l = "0")
    actIKLockSetupPreview(stri, dIKLockIn2, dIKLockIn2, dIKLockOut1, dIKLockOut1)
...
def actIKLockUpdatePreview(index):
    ...
    i_1 = cmds.floatField("ik_li2" + stri, q = True, v = True)
    i_2 = cmds.floatField("ik_li2" + stri, q = True, v = True)
    o_1 = cmds.floatField("ik_lo1" + stri, q = True, v = True)
    o_2 = cmds.floatField("ik_lo1" + stri, q = True, v = True)
...
def fromIKDataAutofillLockData(actionNode):
    ...
    for i in range(len(sSets)):
        ...
        cmds.floatField("ik_li1" + stri, e = True, v = i_2)
        cmds.floatField("ik_li2" + stri, e = True, v = i_2)
        cmds.floatField("ik_lo1" + stri, e = True, v = o_1)
        cmds.floatField("ik_lo2" + stri, e = True, v = o_1)
        ...
        pos1 = int(i_1 / fLength * 200.0)
        pos2 = int((0) / fLength * 200.0)
        pos3 = int((o_1 - i_2) / fLength * 200.0)
        pos4 = int((0) / fLength * 200.0)
        ...
        cmds.setAttr(actionNode + ".IKLockIns1Root[" +strid+ "].IKLockIns1[" +stri+ "]", i_2)
        ...
        cmds.setAttr(actionNode + ".IKLockOuts2Root[" +strid+ "].IKLockOuts2[" +stri+ "]", o_1)
...
def McdExpandExitPNum(actionNode, modNum):
    exitNum = cmds.getAttr(actionNode + ".exitPNum")
    exitNum += modNum
    cmds.setAttr(actionNode + ".exitPNum", exitNum)
...
def McdExpandPreviewSlotNum(actionNode, modNum):
    exitNum = cmds.getAttr(actionNode + ".nbIKLPre")
    exitNum += modNum
    cmds.setAttr(actionNode + ".nbIKLPre", exitNum)
...
def createPhaseDataCurve(dcNode):
    
    # check connection
    allConns = cmds.listConnections(dcNode + ".visibility", d = True, s = False)
    psrNode = ""
    if MIsBlank(allConns):
        # if not create and connect
        psrNode = cmds.createNode("polySplitRing", ss = True)
    ...
    nodeList = [psrNode, psrNode2]

            
    return nodeList
    
    
def actionNotesChange(actNode):
    try:
        dNotes = cmds.textField("ActionComments", q = True, tx = True)
        cmds.setAttr(actNode + ".comment", dNotes, type = "string")
    except:
        try:
            cmds.addAttr(actNode, ln = "comment", dt = "string")
            dNotes = cmds.textField("ActionComments", q = True, tx = True)
            cmds.setAttr(actNode + ".comment", dNotes, type = "string")
        except:
            pass
...
def actionExitTriggerChange(actNode):

    dNotes = cmds.textField("ActionExitTrigger", q = True, tx = True)
    cmds.setAttr(actNode + ".exitTrigger", dNotes, type = "string")
    
    

def setIKLockAttrM(index, setVal, nodeName, isInput):
    ikid = cmds.getAttr(nodeName + ".ikid")
    strid = str(ikid)
    stri = str(index)
    
    if isInput:
        attrName = "IKLockIns2Root"
    else:
        attrName = "IKLockOuts1Root"
    
    if attrName == "IKLockIns1Root":
        cmds.setAttr(nodeName + "." + attrName + "[" + strid + "].IKLockIns1[" + stri + "]", setVal)
    if attrName == "IKLockIns2Root":
        cmds.setAttr(nodeName + "." + attrName + "[" + strid + "].IKLockIns2[" + stri + "]", setVal)
        
    if attrName == "IKLockOuts1Root":
        cmds.setAttr(nodeName + "." + attrName + "[" + strid + "].IKLockOuts1[" + stri + "]", setVal)
    if attrName == "IKLockOuts2Root":
        cmds.setAttr(nodeName + "." + attrName + "[" + strid + "].IKLockOuts2[" + stri + "]", setVal)
        
        
        
def McdSetIK(actNode, ikid, ikName, ikLevel):
    
    # Example: 
    # McdSetIK("walkA_action_casual", 0, "footL", 2)
    # McdSetIK("walkA_action_casual", 1, "footR", 2)
    
    strikid = str(ikid)
    # create ik for action:
    cmds.setAttr(actNode + ".ikid", ikid)
    cmds.setAttr(actNode + ".ikName[" + strikid + "]", ikName, type = "string")
    cmds.setAttr(actNode + ".ikLevel[" + strikid + "]", ikLevel)
    createIKData(actNode, ikid)
        
        

def McdSetIKLock(lockIndex, valueIn, valueOut, actNode):
    
    # Example: 
    # McdSetIKLock(0, 3, 5, "walkA_action_casual")
    # McdSetIKLock(1, 8, 12, "walkA_action_casual")
    # McdSetIKLock(2, 15, 18, "walkA_action_casual")
    
    # fill lock data:
    setIKLockAttrM(lockIndex, valueIn, actNode, True)
    setIKLockAttrM(lockIndex, valueOut, actNode, False)
