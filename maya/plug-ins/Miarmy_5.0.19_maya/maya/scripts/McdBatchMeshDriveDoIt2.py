...
## +
## ===================================================================
##  Module Name: McdBatchMeshDriveDoIt.py
##
##  Description:
##    Apply mesh drive when rendering
##
## ===================================================================
## -
...
def McdBatchMeshDriveDoIt2():
    # hide Miarmy_Contents
    # duplicate
    # enable
    # connect time

    allMDNodes = cmds.ls(type = "McdMeshDrive")
    if allMDNodes == [] or allMDNodes == None:
        ...
        # enable
        McdCreateMeshDriveNode(1)

        # register updater!
        mel.eval("McdSimpleCommand -execute 26");
        
        # set manual mesh drive to true
        globalNode = mel.eval("McdSimpleCommand -exe 2;")
        cmds.setAttr(globalNode + ".manualMeshDrive", 1)
        
        # add to layers if have:
        allRenderLayers = cmds.ls(type = "renderLayer")
        for i in range(len(allRenderLayers)):
            try:
                cmds.editRenderLayerMembers( allRenderLayers[i], 'MDGGRPMASTER', nr = True )
            except:
                pass
